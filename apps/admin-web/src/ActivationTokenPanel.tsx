...
type TokenRow = { id: string; createdAt: string; expiresAt: string | null; status: "Unused" | "Used" | "Expired" | "Revoked"; canCopy: boolean; credentialStatus: "Active" | "Revoked" | null };
  ...
  return <section className="min-w-0 space-y-4 rounded-xl border bg-card p-5 shadow-sm sm:p-6" aria-label="Activation tokens">
    ...
    <Table className="min-w-[620px]"><TableHeader><TableRow><TableHead>Token</TableHead><TableHead>Created</TableHead><TableHead>Expiry</TableHead><TableHead>Status</TableHead><TableHead className="text-right">Actions</TableHead></TableRow></TableHeader>
      <TableBody>{rows.rows.map(token => <TableRow key={token.id}>
        ...
        <TableCell className="py-3.5"><div className="flex flex-col items-start gap-1"><Badge variant="outline" className={statusColors[token.status]}>{token.status}</Badge>{token.status === "Used" && <span className="text-xs text-muted-foreground">{token.credentialStatus === "Active" ? "Access active" : token.credentialStatus === "Revoked" ? "Access revoked" : "Access link unavailable"}</span>}</div></TableCell>
        <TableCell className="py-3.5"><div className="flex justify-end gap-2">
          ...
          {token.status === "Used" && token.credentialStatus === "Active" && <Button type="button" size="icon" variant="ghost" className="text-destructive hover:text-destructive" title="Revoke installation access" aria-label={`Revoke access for token NIQ …${token.id.slice(-6)}`} disabled={disabled || busy} onClick={() => setRevokeTarget(token)}><Ban className="size-4" /></Button>}
    ...
    {rows.rows.some(token => token.status === "Used" && !token.credentialStatus) && <p className="text-sm text-muted-foreground">Older used tokens cannot be linked to an installation automatically. Revoke access from an active used-token row, or disable the deployment to block all installations.</p>}
    ...
    <AlertDialog open={revokeTarget !== null} onOpenChange={open => { if (!open && !busy) setRevokeTarget(null); }}><AlertDialogContent>
      <AlertDialogHeader><AlertDialogTitle>Revoke this installation’s access?</AlertDialogTitle><AlertDialogDescription>Future requests from the installation activated with token NIQ …{revokeTarget?.id.slice(-6)} will be rejected. Other installations stay connected. This cannot be undone.</AlertDialogDescription></AlertDialogHeader>
      <AlertDialogFooter><AlertDialogCancel disabled={busy}>Cancel</AlertDialogCancel><AlertDialogAction disabled={busy} className="bg-destructive text-white hover:bg-destructive/90" onClick={async event => { event.preventDefault(); if (!revokeTarget) return; if (await action(async () => { await request(`/admin/deployments/${deploymentId}/activation-tokens/${revokeTarget.id}/credential`, {}, "DELETE"); })) setRevokeTarget(null); }}>{busy ? "Revoking…" : "Revoke access"}</AlertDialogAction></AlertDialogFooter>
