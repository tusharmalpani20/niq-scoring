...
export function DeleteRecord({ kind, id, name, refresh, revision }: { kind: "clients" | "deployments"; id: string; name: string; refresh: () => Promise<void>; revision: unknown }) {
  ...
  return <>
    {allowed && <Button type="button" variant="ghost" size="sm" className="text-destructive hover:text-destructive" aria-label={`Delete ${name}`} onClick={() => { setError(""); setOpen(true); }}>Delete</Button>}
