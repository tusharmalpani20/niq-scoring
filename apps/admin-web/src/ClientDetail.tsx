...
export function ClientDetail({ data, clientId, refresh }: { data: Overview; clientId: string; refresh: () => Promise<void> }) {
  ...
  return <section className="min-w-0 space-y-4">
    ...
    <Tabs value={activeTab} onValueChange={setActiveTab} className="gap-4"><TabsList variant="line" aria-label="Client details" className="h-auto min-h-11 w-full justify-start gap-6 rounded-none border-b p-0"><TabsTrigger value="overview" className="h-11 flex-none rounded-none border-0 bg-transparent px-1 shadow-none data-[state=active]:bg-transparent data-[state=active]:text-primary after:bottom-0 after:bg-primary">Overview</TabsTrigger><TabsTrigger value="deployments" className="h-11 flex-none rounded-none border-0 bg-transparent px-1 shadow-none data-[state=active]:bg-transparent data-[state=active]:text-primary after:bottom-0 after:bg-primary">Deployments <Badge variant="secondary" className="ml-1 px-1.5 py-0 text-xs">{deployments.length}</Badge></TabsTrigger></TabsList>
      ...
      <TabsContent value="deployments" className="space-y-5"><Card><CardContent className="pt-6"><div className="max-w-full overflow-x-auto"><Table className="min-w-[820px]"><TableHeader><TableRow><TableHead>Deployment</TableHead><TableHead>Hosting</TableHead><TableHead>Status</TableHead><TableHead>Rule</TableHead><TableHead className="text-right">Assessments · all time</TableHead><TableHead className="text-right">Vital IQ · all time</TableHead><TableHead className="text-right">Actions</TableHead></TableRow></TableHeader><TableBody>{deploymentPage.rows.map(item => { const counts = usage?.find(entry => entry.deploymentId === item.id); return <TableRow key={item.id}><TableCell><Link to={deploymentUrl(data, item)} className="font-medium text-foreground hover:underline focus-visible:rounded-sm focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-ring">{deploymentDisplayLabel(item)}</Link><span className="block text-xs capitalize text-muted-foreground">{item.environment} environment</span></TableCell><TableCell>{item.hostingType ? hostingLabels[item.hostingType] : "Not specified"}</TableCell><TableCell><Badge variant={item.enabled ? "default" : "secondary"}>{item.enabled ? "Enabled" : "Disabled"}</Badge></TableCell><TableCell>{deploymentVersion(data, item.id).label}</TableCell><TableCell className="text-right tabular-nums">{counts?.assessments.toLocaleString() ?? "—"}</TableCell><TableCell className="text-right tabular-nums">{counts?.vitalIq.toLocaleString() ?? "—"}</TableCell><TableCell className="text-right"><div className="flex justify-end gap-1"><Button variant="ghost" size="icon" aria-label={`Edit ${deploymentDisplayLabel(item)} deployment`} title="Edit deployment" onClick={() => setSelectedDeployment(item)}><Pencil className="size-4" /></Button><DeleteRecord iconOnly kind="deployments" id={item.id} name={`${client.name} ${deploymentDisplayLabel(item)} deployment`} refresh={refresh} revision={data} /></div></TableCell></TableRow>; })}{deployments.length === 0 && <TableRow><TableCell colSpan={7} className="h-24 text-center text-muted-foreground">No deployments yet.</TableCell></TableRow>}</TableBody></Table></div></CardContent></Card>
