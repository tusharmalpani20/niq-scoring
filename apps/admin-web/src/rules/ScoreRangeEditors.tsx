...
const presetHex: Record<string, string> = { green: "#16A34A", amber: "#D97706", red: "#DC2626", neutral: "#64748B", blue: "#2563EB", purple: "#9333EA" };
...
export function SimpleRiskEditor({ definition, disabled, onChange, drafts, setDrafts }: EditorProps) {
  ...
  return <fieldset disabled={disabled} className="min-w-0 space-y-4 rounded-xl border bg-card p-4 sm:p-5"><div className="flex flex-wrap items-center justify-between gap-3"><div><h2 className="font-semibold">Risk categories</h2><p className="mt-1 text-sm text-muted-foreground">Give each total score a risk category.</p></div><Button type="button" variant="outline" size="sm" disabled={definition.riskCategories.length >= 20} onClick={() => { const id = newRuleId("category"); setEditedRangeId(id); onChange({ ...definition, riskCategories: [...definition.riskCategories, { id, label: "New category", color: "neutral", interpretation: "", min: 0, max: null, minInclusive: true, maxInclusive: true, sources: [] }] }); }}><Plus className="size-4" aria-hidden="true" />Add category</Button></div>
    <div className="divide-y rounded-lg border">{definition.riskCategories.map(category => {
      ...
      const customHex = isHexColor(category.color) ? category.color : null;
      const colorDraftKey = `risk-color:${category.id}`;
      const colorDraft = drafts[colorDraftKey];
      const invalidColorDraft = colorDraft !== undefined && !isHexColor(colorDraft);
      const previewStyle = customHex ? { backgroundColor: `color-mix(in srgb, ${customHex} 12%, var(--background))`, borderColor: `color-mix(in srgb, ${customHex} 45%, var(--border))`, color: "var(--foreground)" } : undefined;
      const clearColorDraft = () => setDrafts(current => Object.fromEntries(Object.entries(current).filter(([key]) => key !== colorDraftKey)));
      ...
      return <div key={category.id} role="group" aria-label={`Risk category ${category.label}`} aria-invalid={rowIssues.length > 0} tabIndex={rowIssues.length ? -1 : undefined} className={`space-y-3 p-3 sm:p-4 ${rowIssues.length ? "border-l-2 border-destructive bg-destructive/5" : ""}`}>
        <div className="flex items-start justify-between gap-3">
          <div><h3 style={previewStyle} className={`inline-flex items-center gap-2 rounded-full border px-2.5 py-1 text-sm font-semibold ${selectedColor?.preview ?? ""}`}><span aria-hidden="true" style={customHex ? { backgroundColor: customHex } : undefined} className={`size-2.5 rounded-full ${selectedColor?.swatch ?? ""}`} />{to === null ? `${from} points and above` : `${from}–${to} points`}</h3><p className="mt-1 text-xs text-muted-foreground">Total assessment score</p></div>
          <Button type="button" variant="ghost" size="icon" className="size-8 shrink-0 text-destructive" aria-label={`Remove ${category.label}`} title={`Remove ${category.label}`} disabled={definition.riskCategories.length <= 1} onClick={() => { clearDraft(`risk:${category.id}:`); clearColorDraft(); if (editedRangeId === category.id) setEditedRangeId(null); onChange({ ...definition, riskCategories: definition.riskCategories.filter(item => item.id !== category.id) }); }}><Trash2 className="size-4" aria-hidden="true" /></Button>
        </div>
        ...
        <div className="space-y-2 text-sm"><p className="text-xs text-muted-foreground">Category color <span className="text-destructive">*</span>{!category.color && " · Neutral default"}</p><Select value={selectedColor?.value ?? "custom"} onValueChange={value => { clearColorDraft(); patch(category.id, { color: value === "custom" ? customHex ?? presetHex[category.color ?? "blue"] ?? "#2563EB" : value }); }}><SelectTrigger aria-label={`Color for ${category.label}`} className="w-44 shadow-none"><span className="flex items-center gap-2"><span aria-hidden="true" style={customHex ? { backgroundColor: customHex } : undefined} className={`size-3 shrink-0 rounded-full ${selectedColor?.swatch ?? ""}`} /><SelectValue>{selectedColor?.label ?? "Custom color"}</SelectValue></span></SelectTrigger><SelectContent>{riskColors.map(color => <SelectItem key={color.value} value={color.value}><span className="flex items-center gap-2"><span aria-hidden="true" className={`size-3 rounded-full ${color.swatch}`} />{color.label}</span></SelectItem>)}<SelectItem value="custom">Custom color…</SelectItem></SelectContent></Select>
          {customHex && <div className="flex flex-wrap items-end gap-2"><label className="space-y-1"><span className="block text-xs text-muted-foreground">Pick color</span><Input type="color" aria-label={`Pick custom color for ${category.label}`} className="h-9 w-12 cursor-pointer p-1" value={customHex} onChange={event => { clearColorDraft(); patch(category.id, { color: event.target.value.toUpperCase() }); }} /></label><label className="space-y-1"><span className="block text-xs text-muted-foreground">Hex color</span><Input className="h-9 w-32 font-mono text-xs shadow-none" aria-label={`Hex color for ${category.label}`} aria-invalid={invalidColorDraft} maxLength={7} placeholder="#2563EB" value={colorDraft ?? customHex} onChange={event => { const value = event.target.value; setDrafts(current => ({ ...current, [colorDraftKey]: value })); if (isHexColor(value)) patch(category.id, { color: value.toUpperCase() }); }} /></label>{invalidColorDraft && <span role="alert" className="text-xs text-destructive">Use # followed by six hex digits.</span>}</div>}
        </div>
