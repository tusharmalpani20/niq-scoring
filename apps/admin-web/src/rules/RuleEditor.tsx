import { decisionText, descriptionText } from "./editor-copy";
...
export function RuleEditor({ initial, onClose, onSaved }: { initial: RuleDetail; onClose: () => void; onSaved: () => void }) {
  ...
  const fixedProfile = isFixedRuleDefinition(record.definition);
  const editable = definition !== null && fixedProfile && ["DRAFT", "VALIDATED"].includes(record.lifecycle);
  useEffect(() => {
    if (!dirty) return;
  ...
  return <><Dialog open onOpenChange={open => { if (!open && !busy) { if (dirty) setConfirm("discard"); else onClose(); } }}><DialogContent className="flex max-h-[94dvh] flex-col overflow-hidden sm:max-w-6xl"><DialogHeader><DialogTitle className="break-words">{record.version}</DialogTitle><DialogDescription><Badge variant="secondary">{record.lifecycle}</Badge><span className="ml-3">{dirty ? "Unsaved changes" : `Revision ${record.revision}`}</span>{!editable && " · Read-only"}</DialogDescription></DialogHeader>
    ...
    {definition && !fixedProfile && <p className="mb-4 text-sm text-muted-foreground">This earlier custom questionnaire is read-only. Create a new version to use the fixed fields and scoring configuration.</p>}
    {definition ? <Tabs value={tab} onValueChange={setTab} className="gap-5"><TabsList variant="line" className="h-auto max-w-full flex-wrap justify-start group-data-[orientation=horizontal]/tabs:h-auto" aria-label="Rule editor sections">{["details", "questionnaire", "scoring", "interventions", "preview", "validation"].map(tab => <TabsTrigger value={tab} key={tab} className="flex-none">{tab.charAt(0).toUpperCase() + tab.slice(1)}</TabsTrigger>)}</TabsList>
      <TabsContent id="rule-tab-details" value="details"><fieldset disabled={!editable || busy} className="space-y-4"><div className="space-y-2"><Label htmlFor="rule-name">Version name</Label><Input id="rule-name" value={definition.name} maxLength={80} onChange={e => update({ ...definition, name: e.target.value })} /></div><div className="space-y-2"><Label htmlFor="rule-description">Description</Label><Textarea id="rule-description" value={descriptionText(definition.description)} onChange={e => update({ ...definition, description: e.target.value })} /></div>
      {definition.issues.length > 0 && <section className="space-y-3"><h3 className="font-medium">Configuration decisions</h3>{definition.issues.map((issue, index) => <div key={issue.id} id={`rule-issues-${issue.id}`} className="space-y-2 rounded-lg border p-4"><p>{decisionText(issue)}</p><Label className="flex gap-2"><input type="checkbox" checked={issue.resolved} onChange={e => update({ ...definition, issues: definition.issues.map((item, i) => i === index ? { ...item, resolved: e.target.checked } : item) })} />Decision resolved</Label><Label htmlFor={`resolution-${issue.id}`}>Resolution and supporting evidence</Label><Textarea id={`resolution-${issue.id}`} value={issue.resolution} onChange={e => update({ ...definition, issues: definition.issues.map((item, i) => i === index ? { ...item, resolution: e.target.value } : item) })} /></div>)}</section>}</fieldset></TabsContent>
