...
export function QuestionnaireEditor({ definition, onChange, disabled }: {
    ...
    {definition.sections.map((s, si) => <section key={s.id} className="space-y-4 rounded-lg border p-4">
      ...
      {s.questions.map((q, qi) => <details key={q.id} className="rounded-md border bg-background p-3"><summary className="cursor-pointer font-medium">{qi + 1}. {q.label || 'Untitled question'} <span className="text-sm font-normal text-muted-foreground">· {types[q.type]}</span></summary><div className="mt-4 space-y-3">
