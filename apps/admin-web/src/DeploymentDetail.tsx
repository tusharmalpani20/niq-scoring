    ...
    <Tabs value={tab} onValueChange={changeTab} className="gap-4"><TabsList variant="line" aria-label="Deployment details" className="h-auto min-h-11 w-full justify-start gap-6 rounded-none border-b p-0"><TabsTrigger value="overview" className="h-11 flex-none rounded-none border-0 bg-transparent px-1 shadow-none data-[state=active]:bg-transparent data-[state=active]:text-primary after:bottom-0 after:bg-primary">Overview</TabsTrigger><TabsTrigger value="settings" className="h-11 flex-none rounded-none border-0 bg-transparent px-1 shadow-none data-[state=active]:bg-transparent data-[state=active]:text-primary after:bottom-0 after:bg-primary">Settings &amp; limits</TabsTrigger><TabsTrigger value="tokens" className="h-11 flex-none rounded-none border-0 bg-transparent px-1 shadow-none data-[state=active]:bg-transparent data-[state=active]:text-primary after:bottom-0 after:bg-primary">Tokens</TabsTrigger></TabsList>
      <TabsContent value="overview" className="space-y-5">
        ...
        <div className="grid items-start gap-4 lg:grid-cols-2">
          <Card><CardContent className="space-y-4 pt-6"><div className="flex flex-wrap items-center justify-between gap-2"><h2 className="font-semibold">Current configuration</h2><Link to={deploymentUrl(data, deployment, "settings")} className="text-sm text-primary hover:underline">Settings &amp; limits <ArrowRight className="inline size-3" aria-hidden="true" /></Link></div><dl className="divide-y text-sm">{[["Environment", environment], ["Hosting", deployment.hostingType ? hostingLabels[deployment.hostingType] : "Not specified"], ["Rule", deploymentVersion(data, deployment.id).label], ["Status", deployment.enabled ? "Enabled" : "Disabled"]].map(([name, value]) => <div key={name} className="flex justify-between gap-3 py-2"><dt className="text-muted-foreground">{name}</dt><dd className="text-right font-medium">{value}</dd></div>)}</dl><Link to={deploymentUrl(data, deployment, "tokens")} className="inline-block text-sm text-primary hover:underline">Manage tokens <ArrowRight className="inline size-3" aria-hidden="true" /></Link></CardContent></Card>
          <Card><CardContent className="space-y-4 pt-6"><div><h2 className="font-semibold">This month’s allowances</h2><p className="mt-1 text-xs text-muted-foreground">Allowance use can include pending requests.</p></div>{allowanceError ? <div role="alert" className="flex flex-wrap items-center gap-2 text-sm text-destructive">Could not load allowances. {allowanceError}<Button variant="outline" size="sm" onClick={() => setAllowanceAttempt(value => value + 1)}>Retry</Button></div> : !dashboardClients ? <p role="status" className="text-sm text-muted-foreground">Loading allowances…</p> : !monthlyAllowance ? <p className="text-sm text-muted-foreground">Monthly allowance data is unavailable.</p> : <div className="divide-y"><AllowanceRow label="Assessments" usage={monthlyAllowance.assessments} /><AllowanceRow label="Face scans" usage={monthlyAllowance.faceScans} /></div>}</CardContent></Card>
        </div>
        {!error && (usage && !hasRecentActivity ? <Card><CardContent className="py-8 text-center"><h2 className="font-semibold">{hasActivity ? "No activity in the last 6 months" : "No completed activity recorded yet"}</h2><p className="mt-1 text-sm text-muted-foreground">{hasActivity ? "Earlier completed activity is included in the all-time totals below." : "Completed assessments and face scans will appear here once this deployment is used."}</p></CardContent></Card> : <Suspense fallback={<p role="status" className="text-sm text-muted-foreground">Loading chart…</p>}><UsageChart monthly={metrics?.monthly ?? null} title="Deployment usage" /></Suspense>)}
        {hasActivity && <p className="text-xs text-muted-foreground tabular-nums">All time: {metrics!.assessments.toLocaleString()} assessments scored · {metrics!.vitalIq.toLocaleString()} face scans completed</p>}
      </TabsContent>
