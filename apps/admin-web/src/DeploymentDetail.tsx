...
export function DeploymentDetail({ data, deploymentId, refresh }: { data: Overview; deploymentId: string; refresh: () => Promise<void> }) {
  ...
  return <section className="min-w-0 space-y-4">
    <header className="space-y-2"><nav aria-label="Breadcrumb" className="text-sm"><Link to="/clients" className="text-primary hover:underline">Clients</Link><span className="mx-2 text-muted-foreground">/</span><Link to={`/clients/${client.id}`} className="text-primary hover:underline">{client.name}</Link><span className="mx-2 text-muted-foreground">/</span><span className="text-muted-foreground">{environment}</span></nav><div className="flex flex-wrap items-center gap-3"><h1 className="text-3xl font-semibold tracking-tight">{environment}</h1><Badge variant={deployment.enabled ? "default" : "secondary"}>{deployment.enabled ? "Enabled" : "Disabled"}</Badge></div><p className="text-sm text-muted-foreground">{client.name} · {deployment.hostingType ? hostingLabels[deployment.hostingType] : "Hosting not specified"} · {deploymentVersion(data, deployment.id).label}</p></header>
    <Tabs value={tab} onValueChange={changeTab} className="gap-4"><TabsList variant="line" aria-label="Deployment details" className="h-auto min-h-11 w-full justify-start gap-6 rounded-none border-b p-0"><TabsTrigger value="overview" className="h-11 flex-none rounded-none border-0 bg-transparent px-1 shadow-none data-[state=active]:bg-transparent data-[state=active]:text-primary after:bottom-0 after:bg-primary">Overview</TabsTrigger><TabsTrigger value="settings" className="h-11 flex-none rounded-none border-0 bg-transparent px-1 shadow-none data-[state=active]:bg-transparent data-[state=active]:text-primary after:bottom-0 after:bg-primary">Settings &amp; limits</TabsTrigger><TabsTrigger value="tokens" className="h-11 flex-none rounded-none border-0 bg-transparent px-1 shadow-none data-[state=active]:bg-transparent data-[state=active]:text-primary after:bottom-0 after:bg-primary">Tokens</TabsTrigger></TabsList>
      ...
      <TabsContent value="settings" className="space-y-4"><div className="flex justify-end"><Button variant="outline" onClick={() => setEditing(true)}><Pencil className="size-4" aria-hidden="true" />Edit deployment</Button></div><div className="grid gap-4 lg:grid-cols-2"><Card><CardContent className="space-y-4 pt-6"><h3 className="border-b pb-4 font-semibold">Configuration</h3><dl className="divide-y text-sm">{[["Client", client.name], ["Environment", environment], ["Hosting", deployment.hostingType ? hostingLabels[deployment.hostingType] : "Not specified"]].map(([label, value]) => <div key={label} className="flex justify-between gap-3 py-3"><dt className="text-muted-foreground">{label}</dt><dd className="text-right font-medium">{value}</dd></div>)}</dl></CardContent></Card><Card><CardContent className="space-y-4 pt-6"><h3 className="border-b pb-4 font-semibold">Rules &amp; limits</h3><dl className="divide-y text-sm">{[["Status", deployment.enabled ? "Enabled" : "Disabled"], ["Rule", deploymentVersion(data, deployment.id).label], ["Assessments", entitlement("SCORING")], ["Vital IQ", entitlement("FACE_SCAN")]].map(([label, value]) => <div key={label} className="flex justify-between gap-3 py-3"><dt className="text-muted-foreground">{label}</dt><dd className="text-right font-medium">{value}</dd></div>)}</dl></CardContent></Card></div></TabsContent>
      <TabsContent value="tokens" className="min-w-0"><ActivationTokenPanel deploymentId={deployment.id} disabled={false} /></TabsContent>
    </Tabs>
