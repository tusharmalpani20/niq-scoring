        ...
        <TabsContent value="details" className="space-y-4">
          <Card className="shadow-none"><CardContent className="p-0">
            <dl className="divide-y">
              ...
              {[["Hosting", hostingLabel], ["Environment", deployment.environment], ["Rule", version]].map(([label, value]) => <div key={label} className="grid grid-cols-[minmax(0,1fr)_minmax(0,2fr)] items-center gap-4 px-4 py-3"><dt className="text-sm text-muted-foreground">{label}</dt><dd className={label === "Environment" ? "break-words text-right capitalize" : "break-words text-right"}>{value}</dd></div>)}
            </dl>
          </CardContent></Card>
          <div className="grid gap-3 sm:grid-cols-2">
            {[["Assessments", "SCORING"], ["Vital IQ", "FACE_SCAN"]].map(([label, capability]) => <Card key={capability} className="shadow-none"><CardContent className="space-y-2 p-4">
