...
test("author option points, caps, guidance and independent sample expectations", async ({ page }) => {
  ...
  expect(saved.scoring[0]).toMatchObject({ kind: "options", questionId: "choice", points: [
    { optionId: "option_a", points: 0 }, { optionId: "option_b", points: 10 },
  ] });
  ...
  await page.getByRole("combobox", { name: "Synthetic choice *", exact: true }).selectOption("option_b");
  ...
  await page.getByRole("button", { name: "Save draft", exact: true }).click();
  await expect(page.getByText("Draft saved.", { exact: true })).toBeVisible();
  expect(state.getRecord()!.definition.samples[0]).toMatchObject({ answers: { choice: "option_b" }, expected: {
