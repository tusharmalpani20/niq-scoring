...
test("deployments show and filter resolved versions and explain the default", async ({ page }, testInfo) => {
  ...
  await expect(dialog.getByRole("list", { name: "Editing progress" })).toBeVisible();
  ...
  await dialog.getByRole("button", { name: "Continue", exact: true }).click();
  await expect(dialog).toContainText("Review & save");
  await expect(dialog).toContainText("125 / month");
