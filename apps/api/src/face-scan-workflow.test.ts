...
test.skipIf(process.env.FACE_SCAN_DATABASE_TEST !== "1")("durable face scans isolate scope, reserve atomically, survive restart and never retry ambiguous dispatch", async () => {
  ...
  try {
    ...
    await workflow.receive(rejected.session.id, "WEBHOOK", normalizeCarePlixResult({ scan_id: `synthetic-${tokenCalls}`, wellness_score: 78, vitals: { heart_rate: 70 } }, `synthetic-${tokenCalls}`));
    expect((await workflow.get(identity, rejected.session.id, "organization-a")).session.state).toBe("FAILED");
    ...
    await workflow.cancel(identity, retry.session.id, "organization-a");
    mode = "success";
    const early = await workflow.create(identity, input("early-callback"));
    await workflow.upload(identity, early.session.id, "organization-a", signal);
    await db`update face_scan_workflows set state='PROCESSING',dispatch_phase='TOKEN',provider_scan_id='synthetic-early' where session_id=${early.session.id}`;
    const earlyResult = normalizeCarePlixResult({ scan_id: "synthetic-early", wellness_score: 78, vitals: { heart_rate: 70 } }, "synthetic-early");
    await workflow.receive(early.session.id, "WEBHOOK", null);
    await workflow.receive(early.session.id, "WEBHOOK", earlyResult);
    expect((await workflow.get(identity, early.session.id, "organization-a")).session.state).toBe("PROCESSING");
    const [earlyUsage] = await db`select outcome from usage_events where id=(select usage_id from face_scan_workflows where session_id=${early.session.id})`;
    expect(earlyUsage?.outcome).toBe("PENDING");
    await db`update face_scan_workflows set dispatch_phase='SUBMIT' where session_id=${early.session.id}`;
    await workflow.receive(early.session.id, "WEBHOOK", earlyResult);
    expect((await workflow.get(identity, early.session.id, "organization-a")).session.state).toBe("COMPLETED");
