...
export async function postgresBindAssessment(database: ReturnType<typeof postgres>, input: BindingInput): Promise<BoundAssessment> {
  return database.begin(async tx => {
    ...
    if (!raw || !versionedRuleDefinitionSchema.safeParse(raw.definition).success || isFinalAssessmentDefinition(raw.definition) && !raw.clinicalUsePermitted) throw new BindingError("VERSION_UNAVAILABLE");
