...
import { validateFinalAssessmentSamples } from "@niq-scoring/scoring-engine/final-assessment";
...
export function installRuleRoutes(app: Hono, store: RuleStore, now: () => Date) {
  ...
  app.get("/admin/rules", guard(async c => c.json({ versions: (await store.list()).map(({ definition, ...record }) => ({ ...record, editable: isFixedVersionedDefinition(definition) && ["DRAFT", "VALIDATED"].includes(record.lifecycle), duplicable: isFixedVersionedDefinition(definition), deletable: versionedRuleDefinitionSchema.safeParse(definition).success && record.lifecycle === "DRAFT" })) })));
  ...
  app.put("/admin/rules/:id", guard(async c => {
    const parsed = z.object({ revision: z.number().int().positive(), definition: versionedRuleDefinitionSchema }).strict().safeParse(await c.req.json().catch(() => null));
    if (!parsed.success) return c.json({ error: "INVALID_RULE_DEFINITION", issues: parsed.error.issues }, 400);
    if (!isFixedVersionedDefinition(parsed.data.definition)) return c.json({ error: "FIXED_RULE_REQUIRED" }, 409);
    const issues = definitionIssues(parsed.data.definition).filter(i => i.severity === "error");
  ...
  app.post("/admin/rules/:id/preview", guard(async c => {
    ...
    const definition = versionedRuleDefinitionSchema.safeParse(record.definition);
    if (!definition.success) return c.json({ error: "LEGACY_RULE_FORMAT" }, 409);
    c.header("Cache-Control", "no-store");
    const result = { ...evaluateVersionedRule(definition.data, parsed.data.answers), ruleVersionId: record.id, checksum: record.packageChecksum, calculatedAt: now().toISOString(), preview: true };
  ...
  app.post("/admin/rules/:id/:action", guard(async c => {
    ...
    const definition = versionedRuleDefinitionSchema.safeParse(record.definition);
    if (!definition.success) return c.json({ error: "LEGACY_RULE_FORMAT" }, 409);
    if (["validate", "approve"].includes(action.data) && !isFixedVersionedDefinition(definition.data)) return c.json({ error: "FIXED_RULE_REQUIRED" }, 409);
    if (["check", "validate", "approve"].includes(action.data)) {
      const issues = [...definitionIssues(definition.data), ...sampleIssues(definition.data)];
