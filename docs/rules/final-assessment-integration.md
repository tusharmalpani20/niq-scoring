...
The Face scan editor stores `faceScanScoring: { ranges: [...] }` in each rule definition. Each range has a stable `id`, percentage `min` and `max`, `minInclusive` and `maxInclusive`, and nonnegative safe-integer `points`. Between 1 and 20 rows must cover every percentage from 0 through 100 exactly once: no gaps, overlaps, duplicate IDs or omitted endpoints. Shared boundaries belong to exactly one adjacent range. Decimal percentages are supported. Added and deleted rows are validated as part of the whole definition.
