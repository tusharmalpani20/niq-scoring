...
4. A fully blank answer set is valid and returns `complete: true`, `score: null`, and `classification: null` with a usage-backed `resultReference`. Render this as an unscored assessment for clinical review. Treat `ASSESSMENT_INCOMPLETE` as non-billable and actionable for unfinished supplied dependencies. Treat `INVALID_ASSESSMENT_ANSWERS` as a client validation error and do not retry unchanged invalid data. Successful responses include immutable version/checksum evidence.
