...
4. A fully blank answer set without a scan is valid and returns `complete: true`, `score: null`, and `classification: null` with a usage-backed `resultReference`. When a completed Vital IQ scan is selected, pass its NIQ Scoring session ID as `faceScanSessionId`; NIQ Scoring verifies it against the deployment, assessment reference, and pinned rule version. The result includes `questionnaireScore`, `faceScan`, and their classified sum in `score`. A blank questionnaire with a scored scan therefore has a final score. An unavailable or mismatched scan returns `FACE_SCAN_UNAVAILABLE` before billing. Treat `ASSESSMENT_INCOMPLETE` as non-billable and actionable for unfinished supplied dependencies. Treat `INVALID_ASSESSMENT_ANSWERS` as a client validation error and do not retry unchanged invalid data. Successful responses include immutable version/checksum evidence.
...
The result includes `ruleVersionId`, an exact `configuration` snapshot, `wellnessScore`, `points` and `status`. `scoringVersion` is `NIQ_FACE_SCAN_2026_09` only for the original mapping and null for custom mappings. Missing/null returns `UNAVAILABLE` with null points; invalid numbers/types are rejected. A real zero score is valid and returns 3 points. No rounding occurs before classification. The Vital IQ mapping is calculated independently; when the application selects that completed scan for assessment scoring, NIQ Scoring adds its points to the questionnaire subtotal and classifies the combined final score.

The Vital IQ editor stores `faceScanScoring: { ranges: [...] }` in each rule definition. Each range has a stable `id`, an optional user-entered `label` (at most 80 characters), numeric 0–100 `min` and `max`, `minInclusive` and `maxInclusive`, and nonnegative safe-integer `points`. The label explains the range to administrators and does not change scoring or the original mapping identifier. Between 1 and 20 rows must cover every score from 0 through 100 exactly once: no gaps, overlaps, duplicate IDs or omitted endpoints. Shared boundaries belong to exactly one adjacent range. Decimal scores are supported. Added and deleted rows are validated as part of the whole definition. The editor shows the range and points first, with boundary details available on demand; this is a presentation change to the same scoring contract.

Historical definitions may omit this property or retain the earlier two-cutoff object (`lowerThreshold`, `upperThreshold`, `belowPoints`, `middlePoints`, `abovePoints`). Both remain valid without rewriting historical JSON or checksums. `normalizeFaceScanScoringConfig` returns a detached row-based view, and the evaluator returns that normalized configuration as result evidence. New templates include explicit default rows. The trusted integration must resolve the selected persisted rule version and pass its configuration and ID to the evaluator; never accept caller-selected settings. This phase adds the calculation and tests only; it does not enable camera capture, authenticate CarePlix, create provider sessions, process callbacks or charge usage. No new public endpoint accepts unverified face-scan claims. Live integration will connect trusted provider results to this evaluator separately.
